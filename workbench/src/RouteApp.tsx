...
import { ArrowDownUp, ArrowUpRight, CarFront, Check, ChevronDown, ChevronRight, GitBranch, Info, LocateFixed, MapPin, Navigation, Pause, Play, Radio, RefreshCw, Search, TriangleAlert, X } from 'lucide-react';
import './route.css';
...
export default function RouteApp() {
  ...
  const road = plan?.routes.find(r => r.id === selectedId); const events = plan?.events || [];
  const locatedCount = events.filter(e => e.locationQuality === 'reported_point').length;
  const manual = !!road && !road.selected;
  const severity = manual || plan?.summary.state === 'review' || plan?.summary.state === 'unavailable';
  const source = object(plan?.provenance.events); const model = object(plan?.provenance.semantic);
  ...
  return <main className="rm-app">
    ...
    <RouteMap origin={stale && plan ? plan.origin : origin} destination={stale && plan ? plan.destination : destination} plan={plan} selectedId={selectedId} stale={stale} showEvents={showEvents} focusEvent={focusEvent} onEvent={openEvents} fitCount={fitCount} />
    <aside className="rm-panel" aria-label="路線規劃">
      <section className="rm-search-card" id="route-inputs">
        <header className="rm-header"><a href="/" aria-label="返回 JEV Studio"><span className="rm-logo"><GitBranch size={22} /></span><strong>JEV <span>Maps</span></strong></a><span className="rm-city">台北・汽車</span></header>
        ...
        {busy ? <button className="rm-cancel" onClick={cancel}>取消等待</button> : <p className="rm-form-note">輸入地點，沿途的公開通報交給路線助理查看。</p>}
      ...
      {plan && road && <section className={`rm-trip-card ${stale ? 'stale' : ''}`} aria-label="本趟路線摘要">
        ...
        {stale ? <div className="rm-assistant warning"><Info size={19} /><div><strong>起終點已更新</strong><p>地圖保留前一條路線，請重新規劃。</p></div></div> : <div className={`rm-assistant ${severity ? 'warning' : ''}`} role="status"><span className="rm-assistant-icon">{severity ? <Info size={19} /> : <GitBranch size={19} />}</span><div><strong>{manual ? '已自行選擇路線' : plan.summary.title}</strong><p>{manual ? `${road.reason || '已切換到所選道路候選'}。自動查看已暫停，這不代表已避開所有事件。` : plan.summary.detail}</p></div></div>}
        <button className="rm-events-row" onClick={() => openEvents()}><span><Radio size={17} />{locatedCount ? '沿途通報' : '通報待核實'} {events.length > 0 && <b>{events.length}</b>}</span><span>查看通報<ChevronRight size={16} /></span></button>
